...
- 新增／編輯共用 `TodoFormFields`（Notion 式 `TodoDateRangePicker`：日期＋結束日期開關＋包含時間開關）
...
- **說明卡層級寫法＋專案一鍵啟動**：CategoryManager 說明卡改 Rola 層級描述（三分鐘看懂、pre-wrap 不橫滑）；番茄頁「專案快捷」＝`isTimeDestination && !required` 維度的葉標籤（不寫死名稱「專案」）；點一下套用最近包含該標籤的組合並 `quickStart`（`canStart` 才啟動），從未用過只帶入該標籤不開始；長按／› 只套用（Pointer Events）。組合走 `selFromTagIds`／`applyCombo` 同源，啟動走既有 `quickStart`。
- **TagGroup 新增 quickStart**（番茄頁快捷按鈕，取代原隱式判斷）；領域維度（`TAG_GROUP_IDS.domain`）三個開關鎖定不可改、視覺特殊化、僅可改名；四個開關說明改為直接顯示；互動面板移除關閉態說明。
- **互動式番茄面板預覽＋刪除鍵防誤按＋葉標籤標題**：標籤管理改重用 `<CategorySelector demoMode showQuickLane={false} />` 即時預覽（獨立 demo state、不寫 combo）；維度「刪」移到標題列最後；番茄主標題改 `tagLeafLabel`（最深層），完整路徑小字 `tagPathLabel`。
...
- ✅ **Z2 標籤管理頁**（群組＋無限層樹編輯器、`isTimeDestination`、軟刪除＋影響範圍）。✅ **Z3 番茄區標籤化**。✅ **說明卡層級寫法＋專案快捷**。✅ **互動預覽＋葉標籤標題**。✅ **quickStart 明確開關＋領域主維度鎖定**。⬜ **Z4～Z7 未執行**。超過三層的降級位置（本批只顯示前三層，供 Z4～Z7 改走標籤樹）：`CAT.cats()`／`categoriesFromDomainTags`（只投影 root→mid→sub）；`SchedulePage` 編輯卡片 cat1/2/3；`SessionHistoryPage`／`CoinHistoryPage` 篩選仍 cat1/2/3；`CalendarPage`／`lib/analytics`／`ReviewView`／`DayReview` `matchesCatSelection`；待辦 `TodoFormFields` 單層 cat；商店／娛樂 `cat1/2/3`；`legacyPath` 只取祖先前三層；`stampSessionCatIds`／`resolveCatIds` 三層名稱仍雙寫。
...
- `tests/`（與 `lib/` 並列）：
  ...
  - `tagSelect.test.ts` — canStart／missingRequired、combo 去重上限 5、toggle 取消、`latestComboContaining` 找不到回 null、未用過專案只帶 `[tagId]`、`isQuickStartGroup`＝`quickStart===true`、`tagLeafLabel` 最深層／空＝未分類／已刪、`demoGroupHints` 只在打開時有文案
...
*最後更新：2026/09/17（quickStart 明確開關＋領域主維度鎖定）*
