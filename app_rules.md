...
| 技術債 #1 班別硬寫死 | **S3-1 資料化＋S3-2 上雲＋S3-3a 取用點資料驅動化完成**——班別自 `DEFAULT_WORKPLACES` 推算上 app_state、`Place` 放寬為 string、取用點改 `placeName()`/`placeShifts()` live；**剩**：S3-3b 編輯 UI（班表 CRUD ＋每日場所選擇器一般化）。 |
