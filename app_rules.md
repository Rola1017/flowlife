...
| 技術債 #1 班別硬寫死 | **S3-1 資料化＋S3-2 上雲＋S3-3a 取用點 live＋S3-3b 跨店多選擋重疊完成**——班別自 `DEFAULT_WORKPLACES` 推算上 app_state、`Place` string、`DayPlan` 改 picks 可跨店、`pickOverlaps` 擋重疊；**剩**：S3-3c 管理工作場所編輯器（班表 CRUD）、S3-3d 單次微調。 |
| dayPlans 尚未上雲 | 每日班別選擇（`day_plans`）目前僅本地、換裝置不同步；觸發＝Rola 開始多裝置排班時，比照 app_state 單例補（`key="day_plans"`）。 |
