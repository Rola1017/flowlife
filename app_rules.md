...
- **金幣頁資訊架構調整**：順序固定為對帳卡 →「① 先選時間區間」→「② 再選要怎麼看」→ 內容（期間永遠先過濾）。收支可點總計卡進獨立收入／支出列表（近→遠）並返回總覽；依分類可點大分類下鑽中分類小計。`coinToast` 顯示延長至 4.6 秒。
...
*最後更新：2026/07/19（金幣頁：時間區間優先＋收支獨立列表＋分類下鑽中分類；toast 4.6s）*
