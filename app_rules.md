...
| 技術債 #1 班別硬寫死 | ✅ **S3 班別使用者化完成**（S3-1~3c-2）：資料化、上雲、跨店 picks、重疊擋、時間/名稱/顏色可編、場所/班別增刪＋孤兒清理、pick 改存班別 id。**剩**：⬜ S3-3d 單次微調（邊緣）。 |
...
- 🔄 **S3 班別設定可編輯**：✅ **S3-1~3c-2 全完成**（資料化、上雲、Place 放寬、DayPlan picks 跨店＋重疊擋、dayPlans/weekSchedule 上雲、WorkplaceManager 時間/名稱/顏色/增刪班別與場所、`pruneOrphanPicks` 孤兒清理、pick 改存班別 id）。⬜ **剩 S3-3d** 單次微調（邊緣）；⬜ **S4** 金鑰移 Edge Functions。

---

*最後更新：2026/06/30（S3-3c-2 增刪工作場所/班別：班別名可改、增刪班別/場所、pick 改存班別 id、pruneOrphanPicks 清孤兒；S3 班別使用者化完成，剩 3c-d 單次微調）*
