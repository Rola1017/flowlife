...
- 🔄 **S3 班別設定可編輯**：✅ **S3-1~3c-2 全完成**（資料化、上雲、Place 放寬、DayPlan picks 跨店＋重疊擋、dayPlans/weekSchedule 上雲、WorkplaceManager 時間/名稱/顏色/增刪班別與場所、`reconcileDayPlans` 孤兒以 label 接回、pick 存班別 id、`findShift` 只認 id；班表改為只顯示明確點選＝`DEFAULT_PLANS` 空＋「🧹 清空所有班別」）。⬜ **剩 S3-3d** 單次微調（邊緣）；⬜ **指定日期例外排程**（未來大功能，見帳本）；⬜ **S4** 金鑰移 Edge Functions。

---

*最後更新：2026/07/01（班表改為只顯示明確點選：DEFAULT_PLANS 改空移除自動預設排班＋SchedulePage 加「🧹 清空所有班別」一鍵清 dayPlans）*
