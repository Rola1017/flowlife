...
│   ├── ColorField.tsx    ← 選色與色碼驗證唯一來源（取色器＋色碼輸入＋預覽；normalizeHex 在 lib/theme.ts）
│   ├── Chip.tsx
│   ├── BackBtn.tsx
│   └── SL.tsx
│
...
lib/
├── theme.ts      ← TH 色彩常數（唯一來源）；withAlpha 為 hex→rgba 唯一實作；normalizeHex 為色碼驗證唯一實作（大寫 #RRGGBB）
...
├── appStateCloud.ts ← app_state 同步；**清單型 key（todos、deleted_todo_ids，見 APP_STATE_LIST_KEYS）一律先合併再推，dirty 不得跳過合併**；非清單型維持 key 級 LWW
...
| `flowlife_v1_todo_tags_migrate` | 待辦標籤遷移結果（本機，**不上雲**、不進 `APP_STATE_KEYS`） |
...
- `endDate`：可執行區間結束日（僅跨日且 `> date` 才存；單日不存）。區間內每天顯示，直到按結束。
...
11. **app_state 雲端同步＝`lib/appStateCloud`** → 金幣明細（`coin_income_log`）整包上雲；餘額由明細加總、不再讀寫獨立 `coins` key（舊 `coins` 僅遷移用）；`useCoinLog` 本地變動才 `pushAppState`、訂閱套回；禁止元件各自直連 supabase 寫 app_state。卡片外框覆寫＝`cardToneColors`（值 `{ [tone]: "#RRGGBB" }`，只存改過的 tone；讀取經 `lib/cardTone` 驗證）。**清單型 key（`APP_STATE_LIST_KEYS`＝todos、deleted_todo_ids）一律先合併再推，dirty 只代表合併後必須推，不得跳過合併。**
...
- **habit-tracker18 批次 J（2026-09-25）**：補齊未上色區塊（直式行程表課程／班別／作息／空檔、課表格、番茄歷史列、金幣列）；卡片色可自訂（app_state `cardToneColors`、設定頁 ColorField、過暗提亮只影響顯示）。Git 交由 Rola 提交。
...
| 決策 | 內容 |
|------|------|
| 待辦主鍵為 `Date.now()` 產生的 number | 兩台裝置同一毫秒新增會撞 id，較新者整筆覆蓋另一筆。機率極低但後果是靜默資料遺失。**觸發＝出現疑似待辦消失、或改用多裝置／匯入資料時**。修法方向＝改用 uuid 主鍵並在合併時保留兩筆。本批不動主鍵（§8-34：不可逆與高風險另批）。 |
| ⬜ 清單型 key 若再增加（例如未來的進度線清單） | 必須加入 `APP_STATE_LIST_KEYS` 並套用同一套「先合併再推」規則；dirty 不得跳過合併。觸發＝新增任何 app_state 清單型 key。 |
...
- ✅ **Z2 標籤管理頁**（群組＋無限層樹編輯器、`isTimeDestination`、軟刪除＋影響範圍）。✅ **Z3 番茄區標籤化**。✅ **說明卡層級寫法＋互動預覽＋葉標籤標題**。✅ **移除專案快捷**。✅ **主維度天藍色＋預覽收折＋維度標題單行**。✅ **Z4 統計與篩選區標籤化**（分攤＋統計維度切換器＋MultiCategoryFilter 吃 tagIds）。✅ **Z5 待辦區標籤化**（表單／卡片／行事曆疊圖篩選／快捷新增預設未分類根／遷移閘門）。⬜ **Z6～Z7 未執行**。超過三層的降級位置（本批只顯示前三層，供 Z6～Z7 改走標籤樹）：`CAT.cats()`／`categoriesFromDomainTags`（只投影 root→mid→sub）；`SchedulePage` 編輯卡片 cat1/2/3；`SessionHistoryPage` 篩選仍可能 cat；商店／娛樂 `cat1/2/3`；`legacyPath` 只取祖先前三層；`stampSessionCatIds`／`resolveCatIds` 三層名稱仍雙寫。待辦舊欄 `cat` 雙寫至 Z8 才刪。
- ⬜ **Z6～Z8 其餘分類標籤化**，見 FlowLife_分類標籤化設計.md §4。
...
- ⬜ **【待測・需 Capacitor App 版】某日詳情頁左右滑動切換日期**（**待 Rola 真機驗收（批次 C 修 touch-action）**）：①背景：網頁版無法測試滑動手勢，需打包成 App 後驗證 ②操作：開啟 FlowLife App → 底部『行事曆』→ 點任一天進入該日詳情頁 → 在頁面空白處用手指由右往左滑 ③過：日期切換到後一天，內容同步更新；由左往右滑切換到前一天；手指上下滑時頁面正常捲動、日期不變 ④沒過：滑動無反應，或上下捲動時日期被誤切換 → 回報是哪一種。
- ⬜ **【待測・需真機】課表行事曆：格子橫捲 vs 標題列換週**（**待 Rola 真機驗收（批次 C 修 touch-action）**）：①操作：底部『課表』→ 在課表格子裡左右滑應只捲內容、週區間不變；在標題列（或格線外）左右滑／點 ‹ › 應切週 ②過：兩者不互相干擾 ③沒過：格子裡滑卻換週，或標題列滑不動。**2026-09-20 已改為 move 達門檻才 capture**；2026-09-22 兩頁收成同一 Hook；批次 C 修 touch-action。
...
### 怎麼跑
- `npm test` — Vitest 單次跑完（CI 同此）
- `npm run test:watch` — 監看模式
- `npm run typecheck` — `tsc --noEmit`（與 CI 獨立步驟相同；不依賴 `next build`）

### 結構
- `vitest.config.ts`：`environment: jsdom`、`globals: true`、`@` → 專案根；讀 `.env.local` 進 `process.env`（契約測試；檔本身被 `.env*` ignore）
- `tests/`（與 `lib/` 並列）：
  ...
  - `appStateList.test.ts` — 清單型 todos 雙向合併含兩邊、墓碑永不復活；tombstones 聯集／同 id 較早 at；property ≥500 任一邊新增合併後不消失
  - `todosApi.test.ts` — `computeAlert` 門檻／過期／無期限、`todoInWindow` 跨日交集與僅 deadline 命中、`toApiTodo` 輸出 tagIds 且雙寫 cat（nowIso 字串鎖死）
  - `todoTagsMigrate.test.ts` — 筆數閘門／孤兒「生活」abort 零寫入／孤兒明細含文字與舊分類名／冪等 written=0／缺領域就補其餘保留／已有領域不動／stampTodoTags 雙寫／快捷新增預設未分類根／ensure 失敗不改 LS
...
### 規則（每批新增測試）
1. **已經出過事的邏輯**優先補回歸；註解標明防的歷史 bug。
2. 期望值＝正確答案，不可把「當前錯誤輸出」貼成 assert。
3. 涉及 localStorage 的測試：`beforeEach` 清空，避免互相污染。
4. 同步／合併等副作用重的流程：抽出純函式再測（如 `mergeSessionsWithTombstones`），測完後行為須與抽出前等價。
5. 本批若改了純函式／邊界條件，應同步加或改對應測試；CI 紅燈不得 `continue-on-error`。
