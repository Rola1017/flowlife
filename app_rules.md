...
| 技術債 #1 班別硬寫死 | **S3-1~S3-3b-2 完成**——班別自 `DEFAULT_WORKPLACES` 推算、workplaces＋dayPlans 皆上 app_state 跨裝置同步、`Place` string、`DayPlan` 改 picks 可跨店、`pickOverlaps` 擋重疊；**剩**：S3-3c 管理工作場所編輯器（班表 CRUD）、S3-3d 單次微調。 |
