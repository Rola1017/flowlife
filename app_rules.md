...
- **歷史頁可改時長/刪除**：`SessionHistoryPage` 每日彙總下列逐顆 session（依 `endTime` 由晚到早）；改時長走 `lib/sessions.setSessionMins`，只動餘額＋該筆 `earnedCoins` 基礎幣、餘額 `Math.max(0,...)` 不為負；里程碑/寶箱不回溯。刪除流程已由「垃圾桶-1」取代：先軟刪、不動金幣，永久刪除才退基礎幣＋清金幣紀錄。無 `id` 舊資料改/刪鈕 disabled。`Session` 有 `manual?` 欄。
...
- **垃圾桶-1 番茄軟刪除（可復原）**：`Session.deletedAt?`＋`LS_KEYS.trashedSessions`；垃圾桶以 app_state `key="trashed_sessions"`（預設 `[]`）做本地＋雲端 LWW 備份。`App.handleDeleteSession` 只用既有 `updateSessions` 把番茄移出 active，再寫入獨立 `trashedSessions`（不退金幣、不清金幣紀錄）；復原移除 `deletedAt`、更新 `updatedAt` 後放回 active；永久刪除有 `window.confirm`，才扣回該筆 `earnedCoins` 並依 uuid 清金幣紀錄。hydrate 時清掉 `deletedAt` 超過 30 天者；重置全部／清除記錄同步清垃圾桶。`SessionHistoryPage` 在手動補番茄後加可折疊「🗑 垃圾桶 (N)」，列日期／分類／名稱／起訖與「↩ 復原」「永久刪除」。已知可接受行為：番茄停留垃圾桶期間，對應金幣紀錄仍暫留，永久刪除才清。
...
*最後更新：2026/07/18（垃圾桶-1：番茄軟刪除、復原／永久刪除、30 天清理、本地＋雲端備份）*
