...
| 分類尚未上雲 | 分類樹（已整棵 id 化）目前僅本地；暫緩原因＝先上番茄/金幣；觸發＝需跨裝置同步分類設定時（比照 appStateCloud 單例 key="categories"）。 |
...
*最後更新：2026/06/29（金幣餘額/記錄上雲 app_state：lib/appStateCloud＋useAppStateCloudSync，(user_id,key) 單例、last-write-wins）*
