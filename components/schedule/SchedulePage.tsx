...
type SchedRow = { id?: string; t: string; n: string; cat1: string; cat2: string; cat3: string; color?: string };
...
export function SchedulePage({
  ...
  const [draft, setDraft] = useState<CourseDraft>({ name: "", cat1: "學習", cat2: "", cat3: "", color: "" });
  type HistoryItem = CourseHistoryItem;
  ...
  const setCell = (d: string, t: string, data: Omit<SchedRow, "t"> | null) =>
    setSched((s) => {
      const prevRows = s[d] || [];
      const existing = prevRows.find((e) => e.t === t);
      const prev = prevRows.filter((e) => e.t !== t);
      return {
        ...s,
        [d]: data ? [...prev, { t, ...data, id: existing?.id ?? data.id ?? newCourseId() }] : prev,
      };
    });
  ...
  const setCells = (targets: EditTarget[], data: Omit<SchedRow, "t"> | null) =>
    setSched((s) => {
      ...
      for (const { d, t } of targets) {
        const prevRows = next[d] || [];
        const existing = prevRows.find((e) => e.t === t);
        const prev = prevRows.filter((e) => e.t !== t);
        next[d] = data ? [...prev, { t, ...data, id: existing?.id ?? data.id ?? newCourseId() }] : prev;
      }
      return next;
    });
            ...
            {clip && (
              <button
                ...
                onClick={() => {
                  const day = dayMenu;
                  if (!day) return;
                  setSched((s) => ({ ...s, [day]: cloneCoursesForPaste(clip.courses) }));
              ...
              <div style={{ borderTop: `1px solid ${TH.border}`, paddingTop: 6, marginTop: 2 }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setSched((s) => {
                      const next = { ...s };
                      for (const d of targets) next[d] = cloneCoursesForPaste(clip.courses);
                      return next;
                    });
                    const skips: string[] = [];
                    if (clip.mode === "full") {
